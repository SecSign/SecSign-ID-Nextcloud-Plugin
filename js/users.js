...
(function(OC, $){
	...
	$(document).ready(function() {
		...
		function impersonateDialog(event) {
			...
			OC.dialogs.prompt(
				t('secsignid', 'Enter a new SecSign ID for "{userId}"?', {userId: userId}),
				t('secsignid', 'Edit SecSign ID' ),
				function(result) {
					if (result) {
						impersonate(userId);
					}
		...
		let registerFunction = function (delay) {
			if(OCA.Settings === undefined) {
				...
				OCA.Settings.UserList.registerAction('icon-user', t('impersonate', 'Edit Secsign ID'), impersonateDialog)
			}
		};
		registerFunction(0);
	});
